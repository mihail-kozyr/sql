Rem
Rem $Header: catexp.sql 09-dec-2004.10:58:04 jgalanes Exp $ expvew.sql
Rem
Rem Copyright (c) 1987, 2004, Oracle. All rights reserved.  
Rem NAME
...
Rem
Rem  MODIFIED
Rem     jgalanes   12/09/04 - Backport 3533996 PDML
Rem     rburns     09/13/04 - check for SYS user 
Rem     jgalanes   12/15/04 - Backport 3651756 
Rem     jgalanes   12/09/04 - Backport 3617574 HAKAN factor 
Rem     bmccarth   04/06/04 - backport 3281907
Rem     kamble     01/16/04 - cleanup garbage due to xmerge tool(exu9pgp)
Rem     kamble     09/30/03 - xbranchmerge to 'st_rdbms_9.2.0.1' 
Rem     jgalanes   11/24/03 - xbranchmerge to 'st_rdbms_9.2.0.1' 
Rem                           Backport 3230116 on top of 9204
Rem     aramarao   05/15/03 - Backport 2900891 on top of 9203
Rem     sasriniv   11/27/02 - Backport 2261722 on top of 9202
Rem     sasriniv   11/22/02 - Backport 2544428 onto 9202
Rem     nireland   09/04/02 - Add select any seq to exp_full_database
Rem     bmccarth   11/22/02 - don't exclude secondary objects from exu9ltts
Rem     jgalanes   05/23/02 - Fix bug 2349201 by exporting objid of data 
Rem                           for trans tablespace.
...
REM V9.0 Table view
REM
REM Notes: tab$.trigflag, objstatus added for 9.0
REM
CREATE OR REPLACE VIEW exu9tabs (
      ...
      AS                                                      /* Heap tables */
       ...
       /******* ÄÎÁÀÂËßÅÒÅ ÂÎÒ ÝÒÓ ÑÒÐÎÊÓ *******/
        AND (o$.type#=2 AND (o$.name NOT IN (SELECT mview_name
                                             FROM dba_mviews)
                            AND                 
                            o$.name NOT IN (SELECT log_table
                                             FROM dba_mview_logs)
                            )
            )
        /* äîáàâëåíî êîíåö */            
      UNION ALL                                         /* Index-only tables */
        SELECT  o$.obj#, o$.dataobj#, o$.name, u$.name, o$.owner#, ts$.name,
                ...
                MOD(TRUNC(o$.flags / 2), 2), t$.property,
                DECODE(BITAND(t$.flags, 32), 32, 1, 0), ts$.dflogging, o$.oid$,
                t$.spare1, DECODE(BITAND(o$.flags, 16), 16, 1, 0), -1, -1, -1,
                t$.flags, t$.trigflag, o$.status
        FROM    sys.tab$ t$, sys.obj$ o$, sys.ts$ ts$, sys.user$ u$,
                sys.com$ c$, sys.ind$ i$
        WHERE   t$.obj# = o$.obj# AND
...
CREATE OR REPLACE VIEW imp9tvoid (
                tname, towner, hash, tvoid, status) AS
        SELECT  o$.name, u$.name, t$.hashcode, t$.tvoid, o$.status
        FROM    sys.obj$ o$, sys.user$ u$, sys.type$ t$
        WHERE   o$.type# = 13 AND
                o$.owner# = u$.user# AND
                t$.toid   = o$.oid$ AND
...
REM
CREATE OR REPLACE VIEW exu9ind_base (
...
CREATE OR REPLACE VIEW exu9snap (
        ...
        /******* ÄÎÁÀÂËßÅÒÅ ÂÎÒ ÝÒÓ ÑÒÐÎÊÓ *******/ and 0=1
/
...
CREATE OR REPLACE VIEW exu8pst (
        ...
        WHERE   u$.name = a$.owner AND
                ...
                t$.obj# = o$.obj#
         /******* ÄÎÁÀÂËßÅÒÅ ÂÎÒ ÝÒÓ ÑÒÐÎÊÓ *******/ 
         and a$.func_package not like 'DBMS_SNAPSHOT%'         
...
CREATE OR REPLACE VIEW exu8fpt (
                ...
                sqlver, property, typobjstatus, tversion, thashcode) AS
                /* Parent types */
        SELECT  o.name, u.name, o.owner#, t.toid,
...
CREATE OR REPLACE VIEW exu92fpt (
     ...
     UNION      /* Subtypes */
        SELECT  sto.name, u.name, sto.owner#, t.toid,
                ...
                sto.status, t.version#, t.hashcode, d.order#
...
CREATE OR REPLACE VIEW exu81tts (
      ...
      AS                                             /* Unpartitioned tables */
        SELECT  t$.dataobj#, o$.name, o$.owner#, NVL(t$.tab#, 0),
      ...
      UNION ALL                                   /* 1st Partition is simple */
        SELECT  0, o$.name, o$.owner#, NVL(t$.tab#, 0), NVL(t$.bobj#, 0),
                ts$.name, t$.spare1
...
REM
REM The following four views are used to determine if a partitionss 
REM subpartition were created using the tables partition template.  They are 
REM referenced by a PL/SQL function in prvtpexp.
REM
CREATE OR REPLACE VIEW sys.exptabsubpart AS
        SELECT 
              tsp.obj#                   OBJNO, 
              tsp.pobj#                  POBJNO,
              tsp.subpart# - 1           SUBPARTNO,
