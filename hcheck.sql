...
REM ======================================================================
REM hcheck8i.sql		Version 1.50	07 Nov 2008
REM
REM Purpose:
 ...
 --
  procedure SeglessUET is
   Cursor cSeglessUET is  
       select u.ts#, u.segfile#, u.segblock#, count(*) cnt, min(ext#) minext#
	 from seg$ s, uet$ u
	where s.ts#(+)=u.ts#
	  and s.file#(+)=u.segfile#
	  and s.block#(+)=u.segblock#
   ...
   loop
     if R.partcnt is null then
       ...
       if (R.comp=0 /*not composite*/) then
         select count(*) into cnt from tabpart$ where bo#=R.obj#;
	 ...
	 loop
	   ...
	   then
		...
		'  - Partition Type# ('||Pobj.type#||')!=19' );
         ...
         select count(*) into cnt from tabcompart$ where bo#=R.obj#;
         if R.partcnt!=cnt then
           if (tag is not null) then hout.put_line(tag); tag:=null; end if;
